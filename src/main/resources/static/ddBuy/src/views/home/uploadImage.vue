<template>
  <div id="collection">
      ...
      />
        ...
        <br>
      <div>
      <van-uploader style="margin-left: 32%;margin-top: 30%" :after-read="afterRead" accept="image/*" >
        <van-button icon="photo" type="primary">上传图片</van-button>
      </van-uploader>
       <van-uploader style="margin-left: 32%;margin-top: 20px"  :after-read="afterRead" capture="camera"  accept="image/*">
        <van-button icon="photo" type="primary">拍照上传</van-button>
      </van-uploader>
      </div>
  ...
  methods: {
    afterRead (file) {
      ...
      if (file.file.size>1 * 1024 * 1024 &&info.userType==0) {
           Notify( {type: 'danger', message: '非会员图片上传不能超过 1M'} );
        return false;
      }

    let formData = new FormData()
    formData.append('file', file.file)
    formData.append('noteType', 1)
    let config = {
      headers: {
